...
class Fatpy(ttk.Frame):
    ...
    def tratar_excel(self,file):
        ...
        for coluna in self.todas_colunas:
            ...
            if df.get(coluna) is None:
                ...
                self.mostrar_msg(f'corrija o nome da coluna para {coluna} ou verifique se existe um cabeçalho')
    ...
    def gerar_fatura_energia(self):
        ...
        self.arquivo_selecionado_copia =  shutil.copy(self.arquivo_selecionado,'./DADOS COPIADOS.xlsx')   
        
        tratado = self.tratar_excel(self.arquivo_selecionado_copia)
        
        if  tratado is False:
            print(' ------- EXCEL NÃO TRATADO ------- ')
            return
        
        df = pd.read_excel(self.arquivo_selecionado_copia)
        ...
        df.to_excel(self.arquivo_selecionado_copia,index=False)
        ...
        for i, (cliente, cnpj, local, lt_atual, lt_anterior, fator, ft_demanda, vl_a_cobrar,ft_bandeira) in enumerate(colunas_necessarias, start=0):
            ...
            print(f'FATURA {nm_fatura}')
                        
            for i in range(linhas):
                self.progress1['value'] += 3
